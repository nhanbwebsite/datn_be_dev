        ...
        if(!$this->collection->isEmpty()){
            ...
            foreach($this->collection as $value){
                ...
                $result['data'][] = [
                    ...
                    'coupon_type'           => $value->coupon->type == 'D' ? 'Giảm tiền trực tiếp trên tổng hóa đơn' : ($value->coupon->type == 'P' ? 'Giảm phần trăm tổng hóa đơn' : 'Miễn phí vận chuyển'),
                    'coupon_value'          => $value->coupon->value ?? null,
                    'coupon_value_formatted' => $value->coupon->type == 'D' ? number_format($value->coupon->value ?? 0).'đ' : ($value->coupon->type == 'P' ? number_format($value->coupon->value ?? 0).'%' : number_format($value->fee_ship ?? 0).'đ'),
