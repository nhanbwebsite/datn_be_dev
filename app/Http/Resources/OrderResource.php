    ...
    public function toArray($request)
    {
        ...
        if(!empty($request)){
            ...
            return [
                ...
                'coupon_type'           => $this->coupon->type == 'D' ? 'Giảm tiền trực tiếp trên tổng hóa đơn' : ($this->coupon->type == 'P' ? 'Giảm phần trăm tổng hóa đơn' : 'Miễn phí vận chuyển'),
                'coupon_value'          => $this->coupon->value ?? null,
                'coupon_value_formatted' => $this->coupon->type == 'D' ? number_format($this->coupon->value ?? 0).'đ' : ($this->coupon->type == 'P' ? number_format($this->coupon->value ?? 0).'%' : number_format($this->fee_ship ?? 0).'đ'),
